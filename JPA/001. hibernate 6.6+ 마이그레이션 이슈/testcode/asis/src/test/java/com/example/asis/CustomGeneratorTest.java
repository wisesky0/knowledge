...
/**
 * AS-IS: Hibernate 5.4.17 Custom Generator 동작 테스트
 * Spring Boot 2.3.3 / javax.persistence
 *
 * Statistics 기반 INSERT/SELECT 횟수 검증 + JDBC Batch 검증
 */
@SpringBootTest
...
class CustomGeneratorTest {
    ...
    @Test
    @DisplayName("tc1_1_직접ID_단건: Custom Generator + 직접 ID 할당, 단건 save")
    @Transactional
    void tc1_1_직접ID_단건() {
        stats.clear();
        TestDataSourceConfig.BATCH_LISTENER.reset();
    ...
    @Test
    @DisplayName("tc1_3_자동생성_단건: Custom Generator + ID 미할당(자동생성), 단건 save")
    @Transactional
    void tc1_3_자동생성_단건() {
        stats.clear();
        TestDataSourceConfig.BATCH_LISTENER.reset();
        ...
        assertThat(TestDataSourceConfig.BATCH_LISTENER.getSelectCount())
            .as("@Version 없음 + ID 자동생성: isNew()=true → persist() → SELECT 없음")
            .isEqualTo(0);
        // JDBC 레벨 SELECT 카운팅 (datasource-proxy)
        int actualSelectCount = TestDataSourceConfig.BATCH_LISTENER.getSelectCount();
    ...
    @Test
    @DisplayName("tc1_5_직접ID_배치10건: Custom Generator + 직접 ID 할당, 10건 saveAll")
    @Transactional
    void tc1_5_직접ID_배치10건() {
        ...
        assertThat(TestDataSourceConfig.BATCH_LISTENER.getSelectCount())
            .as("@Version 없음 + 직접ID 배치10건: merge() 경로로 SELECT 10건 발생")
            .isEqualTo(10);
        ...
        System.out.println("[tc1_5] INSERT=" + stats.getEntityInsertCount()
                + ", SELECT=" + stats.getQueryExecutionCount()
                + ", executeBatch()횟수=" + batchExecCount
    ...
    @Test
    @DisplayName("tc1_6_자동생성_배치10건: Custom Generator + ID 미할당(자동생성), 10건 saveAll")
    @Transactional
    ...
    @Test
    @DisplayName("tc2_1_버전_직접ID_단건: @Version + Custom Generator + 직접 ID 할당, 단건 save")
    @Transactional
    ...
    @Test
    @DisplayName("tc2_3_버전_자동생성_단건: @Version + Custom Generator + ID 미할당, 단건 save")
    @Transactional
    ...
    @Test
    @DisplayName("tc2_5_버전_직접ID_배치10건: @Version + Custom Generator + 직접 ID 할당, 10건 saveAll")
    @Transactional
    ...
    @Test
    @DisplayName("tc_dup1_기존ID_중복저장_단건: 이미 존재하는 ID로 단건 save → @Version 없으면 SELECT→UPDATE(예외 없음)")
    void tc_dup1_기존ID_중복저장_단건() {
        ...
        // 2. 동일 ID로 두 번째 저장 시도
        // AS-IS (@Version 없음): isNew() 판단 위해 SELECT 수행 → 존재 확인 → merge → UPDATE
        // 예외 없이 UPDATE 실행됨
        transactionTemplate.execute(status -> {
        ...
        // SELECT 1회 후 UPDATE 수행 (INSERT 시도 없음)
        // getQueryExecutionCount()는 JPQL/HQL만 카운트, PK 조회는 getEntityLoadCount()로 확인
        ...
        assertThat(TestDataSourceConfig.BATCH_LISTENER.getSelectCount())
            .as("AS-IS @Version 없음: PK SELECT 1건 발생 (JDBC 레벨)")
            .isEqualTo(1);
        assertThat(stats.getEntityLoadCount())
            .as("AS-IS @Version 없음: PK SELECT로 isNew() 판단 → entityLoad=1")
            .isEqualTo(1);
        assertThat(stats.getEntityUpdateCount())
            .as("AS-IS @Version 없음: 기존 엔티티 발견 시 merge → UPDATE=1")
            .isEqualTo(1);
        assertThat(stats.getEntityInsertCount())
            .as("AS-IS @Version 없음: INSERT 시도 없음")
            .isEqualTo(0);
    }
    ...
    void tc_dup2_버전_기존ID_중복저장_단건() {
        ...
        // 2. 동일 ID로 두 번째 저장 시도
        // AS-IS (@Version 있음): version=0 이면 isNew()=true → persist() → SELECT 없이 INSERT 시도 → PK 중복 오류
        ...
        assertThat(stats.getQueryExecutionCount())
            .as("AS-IS @Version 있음: SELECT 없이 INSERT 시도 → DB PK 중복 오류 (SELECT=0)")
            .isEqualTo(0);
        assertThat(TestDataSourceConfig.BATCH_LISTENER.getSelectCount())
            .as("AS-IS @Version 있음: JDBC 레벨 SELECT 없음 (SELECT=0)")
            .isEqualTo(0);
    }
    ...
    void tc_opt1_낙관적잠금_버전충돌() {
        ...
        // 2. Tx-A: 조회 (version=0 스냅샷 획득, 아직 커밋 안 함)
        ArticleVersioned staleSnapshot = transactionTemplate.execute(status -> {
            ...
            // detach 하여 영속성 컨텍스트 밖으로 꺼냄 (stale 상태 유지)
        ...
        System.out.println("[tc_opt1][AS-IS] 실제 예외 클래스: " + thrown.getClass().getName());
        System.out.println("[tc_opt1][AS-IS] 원인 예외 클래스: " + (thrown.getCause() != null ? thrown.getCause().getClass().getName() : "null"));
    ...
    @Test
    @DisplayName("tc2_6_버전_자동생성_배치10건: @Version + Custom Generator + ID 미할당, 10건 saveAll")
    @Transactional
